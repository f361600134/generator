com.kepe.dragon.persistent.provider

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import com.kepe.asyncdb.PersistentAddProvider;
import com.kepe.dragon.persistent.dao.ArtifactDAO;
import com.kepe.dragon.persistent.domain.Artifact;

/**
* @author Jeremy
*/
@Component
public class ArtifactPersistentProvider implements PersistentAddProvider<Artifact>{

	@Autowired
	private ArtifactDAO dao;
	
	@Override
	public void save(Artifact data) throws Exception {
		if(data!=null) {
			if (data.isDirty()) {
				data.setDirty(false);
				data.beforeSave();
				dao.insert(data);
			}
			else 
				dao.update(data);
		}
	}

	@Override
	public void add(Artifact data) throws Exception {
		this.save(data);
	}
	
}
